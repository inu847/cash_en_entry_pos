        ...
        <div class="nav-container">
            <nav id="main-menu-navigation" class="navigation-main">
                ...
                <div class="nav-item {{ ($segment1 == 'pos') ? 'active' : '' }}">
                    <a href="{{ route('pos.index') }}"><i class="ik ik-printer"></i><span>{{ __('POS')}}</span> <span class=" badge badge-success badge-right">{{ __('New')}}</span></a>
                </div>
                <div class="nav-item {{ ($segment1 == 'products') ? 'active open' : '' }} has-sub">
                    <a href="#"><i class="ik ik-headphones"></i><span>{{ __('Products')}}</span></a>
                    <div class="submenu-content">
                        <a href="{{ route('products.create') }}" class="menu-item {{ ($segment1 == 'products' && $segment2 == 'create') ? 'active' : '' }}">{{ __('Add Product')}}</a>
                        <a href="{{ route('products.index') }}" class="menu-item {{ ($segment1 == 'products' && $segment2 == '') ? 'active' : '' }}">{{ __('List Producs')}}</a>
                    </div>
                </div>
                <div class="nav-item {{ ($segment1 == 'categories') ? 'active' : '' }}">
                    <a href="{{ route('product-category.index') }}"><i class="ik ik-list"></i><span>{{ __('Categories')}}</span></a>
                </div>
                <div class="nav-item {{ ($segment1 == 'warehouse') ? 'active' : '' }}">
                    <a href="{{ route('warehouse.index') }}"><i class="ik ik-list"></i><span>{{ __('Warehouse')}}</span></a>
                </div>
