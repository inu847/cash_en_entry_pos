<form class="forms-sample" method="POST" action="{{ route('transferIn.update', [$data->id]) }}" enctype="multipart/form-data">
    @method('PUT')
    @csrf
        <div class="row">
            <div class="col-sm-12">
                ...
                <div class="form-group">
                    ...
                    <select class="form-control" name="from_warehouse_id" >
                        ...
                        @foreach($fromwarehouse as $value)
                            <option value="{{$value->id}}" @if($value->id == $data->from_warehouse_id) selected @endif>{{$value->name}}</option>
                        @endforeach
                    </select>
                </div>  
                ...
                <div class="form-group">
                    ...
                    <select class="form-control" name="to_warehouse_id" >
                        <option selected="selected" value="" >Select Warehouse</option>
                        @foreach($towarehouse as $value)
                            <option value="{{$value->id}}" @if($value->id == $data->to_warehouse_id) selected @endif>{{$value->name}}</option>
                        @endforeach
                    </select>
                </div>  
